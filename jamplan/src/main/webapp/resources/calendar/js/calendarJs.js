			...
			$.each(str, function (index, item) {
				...
				if (yearSub == year && monthSub == Number(month)) {
					...
					if ($('#userId' + day).length == 0) {
						...
						output += String(item.id) + "<br/>" + "</div>";
						$("#bind" + day).append(output);
					} else if ($('#userId' + day).length != 0) {
						var output = String(item.id) + '<br/>';
			...
			$.each(str, function (index, item) {
				...
				if (yearSub == year && monthSub == Number(month)) {
					...
					if (item.confirmIndicator == 1) {
						$("#heart" + day).css({ "background": "url('/jamplan/resources/calendar/img/confirm.jpg')" });
					}
				}
			});
		},
	...
	$(".goodBut").on("click",function() {
					...
					//저장하기 버튼이 있고, 한번 눌렀을 때
						...
						if ($("#bind" + num).css("background-color") != "rgb(71, 178, 151)") {
							//바탕색을 초록색으로 바꾸기
							$("#heart" + num).css({ "background-image": 'url("/jamplan/resources/calendar/img/confirm.jpg")' });
						...
						//만약 누른 버튼의 바탕 색이 초록색이면 (이미 확정난 날이면 - 저장 취소)
							...
							//배경색 흰색으로 
							$("#heart" + num).css({ "background-image": 'url("/calendar/img/heart.jpg")' });
							//ajax로 선택한 날 서버로 전송
							$.ajax({
									url : "fixcal.mp",
									type : "POST",
	...
	for (var day = 1; day <= totMonth; day++) {
		...
		if (dt.getDay() != 6) {
			html += '<td class = "date" id = "dateTd' + String(day)
				...
				+ '<button class= "goodBut btn btn-outline-light text-dark" id = "dateButton' + String(day)
...
// 달력 출력 함수 <h2 style="width: 920px; text-align:center;">
function printYearMonth(year, month) {
	var html = '<h1 style="width: 580px; text-align:center; margin-top:10px">' + String(year) + '년 ' + String(month) + '월' + '</h1>';
	// console.log(html);
